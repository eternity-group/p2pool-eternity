...
import p2pool
from . import data as eternity_data
...
class Protocol(p2protocol.Protocol):
    ...
    message_inv = pack.ComposedType([
        ('invs', pack.ListType(pack.ComposedType([
            ('type', pack.EnumType(pack.IntType(32), {1: 'tx', 2: 'block', 3: 'filtered_block', 4: 'txlock_request', 5: 'txlock_vote', 6: 'spork', 7: 'eternitynode_winner', 8: 'eternitynode_scanning_error', 9: 'evolution_vote', 10: 'evolution_proposal', 11: 'evolution_finalized', 12: 'evolution_finalized_vote', 13: 'eternitynode_quorum', 14: 'eternitynode_announce', 15: 'eternitynode_ping'})),
    ...
    message_tx = pack.ComposedType([
        ('tx', eternity_data.tx_type),
    ])
    def handle_tx(self, tx):
        self.factory.new_tx.happened(tx)

    message_block = pack.ComposedType([
        ('block', eternity_data.block_type),
    ])
    def handle_block(self, block):
        block_hash = self.net.BLOCKHASH_FUNC(eternity_data.block_header_type.pack(block['header']))
    ...
    message_block_old = pack.ComposedType([
        ('block', eternity_data.block_type_old),
    ])
    ...
    message_headers = pack.ComposedType([
        ('headers', pack.ListType(eternity_data.block_type_old)),
    ])
    def handle_headers(self, headers):
        for header in headers:
            header = header['header']
            self.get_block_header.got_response(self.net.BLOCKHASH_FUNC(eternity_data.block_header_type.pack(header)), header)
